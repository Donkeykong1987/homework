'''Stwórz prostą symulację interakcji Klient-Serwer przy użyciu
klas.
...
class FakeClient:
    def send(self, server: FakeServer, request: dict):
        print(f"--- Wysyłam żądanie: {request}")
...
client.send(server, {
    "method": "POST",
    "target": "/users",
    "body": {"id": 3, "name": "Paweł"}
})


client.send(server, {"method": "GET", "target": "/unknown"})